...
前端仪表盘中的“当前版本”现在**不再手工写死**，而是构建时自动生成：

1. 若当前构建命中 Git tag，则显示该 tag（例如 `v3.0.6`）
2. 若当前构建不在 tag 上，则回退显示当前 commit 短哈希（例如 `a75714f`）
3. 若 Git 信息不可用，则显示 `unknown`
...
可以自然保持一致，不需要再手改前端 `.env.production`。

### 当前推荐镜像

- Docker: `ray5378/cloudpan189pro:latest`
- 固定版本示例：`ray5378/cloudpan189pro:3.0.6`
...
### 规则 B：每天中午仅对 0 文件节点做一次判定
当前实现已经收紧为：

- **每天仅在中午 `12:00` 执行一次**
- 先筛选出 **文件数量 = 0** 的节点作为候选对象
- 只对这些候选对象继续查询最新任务日志并做删除判定
