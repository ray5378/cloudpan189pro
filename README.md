...
- 每隔 **10 分钟** 自动执行一轮**深度刷新**
- 总共执行 **6 轮**
- 只有当这 **6 轮结果都满足：刷新成功，且文件数量仍然 = 0**，才会最终删除该节点

任意一轮出现以下任一情况，就会取消删除流程：
...
这样可以尽量排除网络波动、短时异常、云盘接口抖动带来的误删。

### 删除前日志
自动删除前会先记录一条任务日志，写明命中原因，例如：

- `命中自动删除关键词: 已失效`
- `未启用自动刷新或已过期，且连续6轮深度刷新（每10分钟一轮）均成功且文件数量仍为0`
