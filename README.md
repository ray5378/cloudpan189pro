...
- 自动删除失效存储：支持“关键词命中”或“未启用自动刷新/已过期且 0 文件并最新刷新成功”两类规则，满足任一条件即可删除
...
前端仪表盘中的“当前版本”改为**构建时读取前端环境变量**：

- 默认从 `fe/.env.production` 中的 `VITE_APP_VERSION` 读取
- 若未提供，则回退显示 `unknown`

当前默认值示例：

```env
VITE_APP_VERSION=v3.0.11
```

这样做的原因是：部分部署环境无法稳定访问 `.git` 元数据，因此使用显式版本变量更稳。
发版时只需要同步更新：
- Git tag
- Docker tag
- `VITE_APP_VERSION`
...
- Docker: `ray5378/cloudpan189pro:latest`
- 当前固定版本：`ray5378/cloudpan189pro:3.0.11`
- Git tag：`v3.0.11`
...
### 规则 B：未启用自动刷新/已过期且 0 文件
当前实现为：

- **每天仅在中午 `12:00` 执行一次**
- 对存储节点统一检查删除规则
- 满足 **规则 A 或规则 B 任一条件** 即可删除
...
此外，系统设置页支持“**手动执行一次**”按钮，手动触发与定时自动删除使用相同判断逻辑。

### 删除前日志
自动删除前会先记录一条任务日志，写明命中原因，例如：

- `命中自动删除关键词: 已失效`
- `未启用自动刷新或已过期，且文件数量为0、最新刷新成功`
