...
func WithFileRotationP(file string) Option {
	...
	return func(opt *option) {
		opt.file = &lumberjack.Logger{ // concurrent-safed
			...
			MaxBackups: 0,     // 不限制备份数量；仅按 MaxAge 删除
			MaxAge:     15,    // 保留 15 天，超期直接删除
			LocalTime:  true,  // 使用本地时间
			Compress:   false, // 不压缩，超期直接删除
		}
	}
}
...
func NewJSONLogger(opts ...Option) (*zap.Logger, error) {
	...
	encoderConfig := zapcore.EncoderConfig{
		...
		CallerKey:     "caller",
		MessageKey:    "msg",
		StacktraceKey: "stack_trace", // use by zap.AddStacktrace; optional; useless
...
func WrapMeta(err error, metas ...Meta) (fields []zap.Field) {
	...
	for _, mt := range metas {
		fields = append(fields, zap.Any(mt.Key(), mt.Value()))
	}
