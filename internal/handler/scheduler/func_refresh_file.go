...
func buildSchedulerPanClient(session *appsessionSvi.Session) *cloudpan.PanClient {
	if session == nil {
		return nil
	}
	...
	for {
		...
		case <-ticker.C:
			...
			for id := range s.lastTriggeredSlot {
				if _, ok := activeMountIDs[id]; !ok {
					delete(s.lastTriggeredSlot, id)
				}
			}
			for fid := range s.autoDeleteConfirmations {
				if _, ok := activeFileIDs[fid]; !ok {
					delete(s.autoDeleteConfirmations, fid)
				}
			}
			s.mu.Unlock()
