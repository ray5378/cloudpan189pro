...
## 3.2.1 CAS 配置页前端开发规范（新增，后续必须遵守）

这一节是专门给后续维护 CAS 配置页的人和 AI 写的，目的是防止页面再次退化成单文件大泥坑。

### 页面结构约束

CAS 配置页必须保持“页面编排层 + 功能卡片 + 动作组合逻辑”三层结构：

- `index.vue`
  - 只负责页面编排
  - 只保留跨卡片共享的状态、计算属性、跨区联动逻辑
- `components/*Card.vue`
  - 每个功能区单独拆分
  - 例如：来源目录、默认配置、手动恢复、请求预览、链路说明、恢复结果、缓存操作
- `composables/useXxx.ts`
  - 承载带确认弹窗、loading、消息提示、可复用的动作逻辑
  - 例如：清空缓存表、重建缓存表
...
- 把新增按钮、弹窗、接口调用、loading 状态直接继续堆进 `index.vue`
- 把多个功能区重新揉回一个超长单文件组件
- 通过多次零碎补丁在同一 SFC 中追加重复 script/style/template 片段
- 为了“先能跑”而放弃拆分，留下明显技术债
...
任何对 CAS 配置页的改动，在提交前至少满足：

- 前端 `npm run build` 通过
- 不存在未使用 import / 未使用状态 / 残留调试逻辑
- 不存在重复 `<script>` / `<style>` / 模板残片
- 页面仍然保持按功能区拆分，而不是回退成单文件大泥坑

### 给 AI 的一句话约束

如果你是 AI，在修改 CAS 配置页时：

> 先判断功能属于哪个卡片；能拆就拆，能复用就抽 composable；不要为了省事把逻辑重新堆回 `index.vue`。

## 3.2.2 订阅 `.cas` 自动转存失败后的延迟重试规则（新增）

这一条专门描述“订阅扫描链里 `.cas` 自动转存”的 SHARE_SAVE 重试语义，避免后续误改成同步 sleep、无限重试或立即连打。
...
不等同于所有手动恢复任务，也不等同于其他媒体/STRM 任务。
...
云盘侧任务失败很多时候不是瞬时网络问题，而是任务态短时间不稳定。立刻连续重提往往只会放大失败，因此这里固定为：
